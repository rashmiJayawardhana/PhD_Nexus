// FILE: src/components/content/chapter04/05WasteDisposalSection.tsx (Updated)
      ...
      >
        <div className="space-y-5">
          ...
          <div className="bg-linear-to-br from-red-50 to-orange-50 rounded-xl p-6 border-2 border-red-300 shadow-lg">
            ...
            <div className="bg-white rounded-lg p-5 mb-4">
              <ul className="space-y-2 text-sm text-slate-700">
                {data.wasteTypes.map((type: string, idx: number) => (
            ...
            {/* Waste Category Cards - NOT editable (visual organization) */}
          ...
          <div className="bg-white rounded-xl p-6 border-2 border-slate-300 shadow-md mt-6">
            ...
            <RequirementsList
              items={data.handlingPractices}
              color="red"
            />
          </div>

          <InfoBox type="note">
            <strong>Patient Excreta:</strong> {data.excretaNote}
          </InfoBox>
          ...
          <div className="bg-linear-to-br from-blue-50 to-cyan-50 rounded-xl p-6 border-2 border-blue-300 shadow-md">
            ...
            <div className="grid md:grid-cols-2 gap-4">
              <ul className="space-y-2 text-sm text-slate-700">
                {data.containerRequirements.slice(0, 2).map((req: string, idx: number) => (
                  <li key={idx} className="flex items-start gap-2">
              ...
              <ul className="space-y-2 text-sm text-slate-700">
                {data.containerRequirements.slice(2).map((req: string, idx: number) => (
                  <li key={idx} className="flex items-start gap-2">
                    <span className="text-blue-600 font-bold">✓</span>
                    <span>{req}</span>
                  </li>
          ...
          <div className="bg-linear-to-br from-amber-50 to-yellow-50 rounded-xl p-6 border-2 border-amber-300 mt-6">
            ...
            <ul className="space-y-2 text-sm text-slate-700">
              {data.storageRequirements.map((req: string, idx: number) => (
                <li key={idx} className="flex items-start gap-2">
          ...
          <div className="bg-linear-to-br from-purple-50 to-pink-50 rounded-xl p-6 border-2 border-purple-300 mt-6">
            ...
            <ul className="space-y-2 text-sm text-slate-700">
              {data.excretaHandling.map((practice: string, idx: number) => (
                <li key={idx} className="flex items-start gap-2">
